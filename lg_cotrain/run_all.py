...
def run_all_experiments(
    ...
    *,
    ...
    Args:
        task: Task name (informative, humanitarian, damage).
    ...
    with ``(task, modality, budget, seed_set, status)`` where *status* is one of
...
def format_summary_table(all_results, task, modality, budgets=None, seed_sets=None):
    ...
    lines = []
    lines.append(f"=== Results for {task}/{modality} ===")
    lines.append("")
...
def main():
    parser = argparse.ArgumentParser(
        description="Run all (budget x seed_set) experiments for one task/modality"
    )
    parser.add_argument(
    ...
    print()
    print(format_summary_table(all_results, args.task, args.modality,

...
def _run_single_experiment(config_kwargs: dict) -> dict:
    ...
    Returns:
        Dict with keys: task, modality, budget, seed_set, status, result.
    """
    ...
    try:
        ...
        logging.getLogger("lg_cotrain").error(
            f"Experiment {task}/{modality} budget={budget} seed={seed_set} failed: {e}"
        )
        return {
            "task": task,
            "modality": modality,
...
def run_experiments_parallel(
    ...
    Args:
        ...
        on_experiment_done: Optional callback(task, modality, budget, seed_set, status)
            invoked in the parent process after each experiment completes.

    Returns:
        List of outcome dicts (with keys task, modality, budget, seed_set,
        status, result), in the same order as *experiment_configs*.
    """
    if num_gpus < 1:

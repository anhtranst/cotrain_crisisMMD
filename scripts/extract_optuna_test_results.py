...
The directory structure is:
    {target}/{budget}_set{seed}/trials_{n}/best_params.json
    {target}/{budget}_set{seed}/trials_{n}/study.log

Usage:
    python scripts/extract_optuna_test_results.py \\
        --target results/optuna/per_experiment/humanitarian/text_only --n-trials 10

    python scripts/extract_optuna_test_results.py \\
        --target results/optuna/per_experiment/humanitarian/text_only --n-trials 10 \\
        --output results/optuna/per_experiment/humanitarian/text_only/best_results.json
"""
...
def collect_all_test_metrics(experiment_dir):
    ...
    Args:
        experiment_dir: Path to an experiment directory
            (e.g. results/optuna/per_experiment/humanitarian/text_only/5_set1/)
...
def extract_results(results_dir, n_trials=None):
    ...
    Walks: results_dir / {budget}_set{seed} /

    Returns:
        List of result dicts sorted by (budget, seed_set).
    """
    ...
    for exp_dir in sorted(results_dir.iterdir()):
        ...
        if test_metrics is None:
            print(
                ...
                f"in budget={budget} seed={seed_set}",
                file=sys.stderr,
            )
...
def main():
    ...
    parser.add_argument(
        ...
        help="Path to per_experiment results directory (e.g. results/optuna/per_experiment/humanitarian/text_only)",
    )
    parser.add_argument(

...
Usage:
    ...
    python merge_optuna_results.py --target results/optuna/per_experiment/humanitarian/text_only --n-trials 10
...
TOTAL_STUDIES = len(BUDGETS) * len(SEED_SETS)  # 12 per task/modality


def _study_path(base_dir: Path, budget: int, seed: int, n_trials: int) -> Path:
    return base_dir / f"{budget}_set{seed}" / f"trials_{n_trials}"
    ...
    for source_dir in sources:
        ...
        for budget in BUDGETS:
            for seed in SEED_SETS:
                src_trial_dir = _study_path(source, budget, seed, n_trials)
                src_params = src_trial_dir / "best_params.json"
                if not src_params.exists():
                    continue

                dst_trial_dir = _study_path(target, budget, seed, n_trials)
                ...
                if dry_run:
                    print(f"  [dry-run] Would copy: b={budget} s={seed}")
                    stats["copied"] += 1
                    continue
...
def generate_summary(target: Path, n_trials: int) -> dict:
    ...
    for budget in BUDGETS:
        for seed in SEED_SETS:
            params_path = _study_path(target, budget, seed, n_trials) / "best_params.json"
...
def print_report(summary: dict, missing: list, merge_stats: dict = None):
    ...
    if missing and n_missing <= 30:
        print(f"\n  Missing studies:")
        for budget, seed in missing:
            print(f"    b={budget} s={seed}")
...
def main():
    ...
    parser.add_argument(
        ...
        help="Target directory (e.g., results/optuna/per_experiment/humanitarian/text_only)",
    )
    parser.add_argument(

...
Usage:
    ...
    python scripts/check_progress.py --results-dir /path/to/results
    python scripts/check_progress.py --watch          # refresh every 30s
    python scripts/check_progress.py --watch --interval 10
    python scripts/check_progress.py --num-gpus 2     # override GPU count for ETA
"""
...
def parse_study_log(log_path: str) -> dict:
    ...
    info = {
        "path": log_path,
        "label": None,          # e.g. "humanitarian/text_only b=5 s=1"
        "budget": None,
        "seed": None,
        "target_trials": 0,
        "completed_trials": 0,
    ...
    try:
        with open(log_path, "r", encoding="utf-8", errors="replace") as f:
            for line in f:
                ...
                # Study header — new format: "task/modality budget=N seed=N"
                m = re.search(
...
def print_progress(results_dir: str, num_gpus_override: int = None,
                    total_studies_override: int = None):
    ...
    # Total studies: use override, or count from logs
    total_studies = total_studies_override or len(studies)
    ...
    if in_progress:
        ...
        for s in in_progress:
            name = f"{s['label'] or '?'} b={s['budget']} s={s['seed']}"
    ...
    if completed:
        ...
        by_label = {}
        for s in completed:
            by_label.setdefault(s["label"] or "unknown", []).append(s)

        for label in sorted(by_label):
            entries = sorted(by_label[label], key=lambda x: (x["budget"], x["seed"]))
            avg_f1 = sum(s["best_f1"] for s in entries) / len(entries)
            print(f"    {label:<40} [{len(entries)}] avg_best_f1={avg_f1:.4f}")
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Total number of studies expected (default: auto-detect from logs)",
    )
    ...
    if args.watch:
        try:
            while True:
                ...
                print_progress(args.results_dir, args.num_gpus, args.total_studies)

...
class TestTaskLabels(unittest.TestCase):
    ...
    def test_all_sorted(self):
        for task, labels in TASK_LABELS.items():
            self.assertEqual(labels, sorted(labels), f"{task} labels not sorted")

    def test_default_is_humanitarian(self):
        self.assertEqual(CLASS_LABELS, TASK_LABELS["humanitarian"])
...
class TestBuildLabelEncoder(unittest.TestCase):
    ...
    def test_affected_individuals_is_zero(self):
        """First alphabetically should be id 0."""
        label2id, _ = build_label_encoder()
        self.assertEqual(label2id["affected_individuals"], 0)
    ...
    def test_custom_labels_roundtrip(self):
        ...
        subset = ["affected_individuals", "infrastructure_and_utility_damage", "not_humanitarian"]
        label2id, id2label = build_label_encoder(labels=subset)
        for label in subset:
            self.assertEqual(id2label[label2id[label]], label)
...
class TestDetectClasses(unittest.TestCase):
    """Test detect_classes with list-of-dicts (pure-Python path)."""

    def test_single_list(self):
        records = [
            {"class_label": "not_humanitarian"},
            {"class_label": "affected_individuals"},
            {"class_label": "not_humanitarian"},
        ]
        result = detect_classes(records)
        self.assertEqual(result, ["affected_individuals", "not_humanitarian"])

    def test_union_of_multiple_lists(self):
        list1 = [{"class_label": "affected_individuals"}]
        list2 = [{"class_label": "not_humanitarian"}]
        list3 = [{"class_label": "vehicle_damage"}]
        result = detect_classes(list1, list2, list3)
        self.assertEqual(result, [
            "affected_individuals", "not_humanitarian", "vehicle_damage",
        ])
    ...
    def test_overlapping_lists(self):
        ...
        list2 = [
            {"class_label": "not_humanitarian"},
            {"class_label": "vehicle_damage"},
        ]
        result = detect_classes(list1, list2)
        self.assertEqual(result, [
            "affected_individuals", "not_humanitarian", "vehicle_damage",
        ])
    ...
    def test_empty_input(self):
        result = detect_classes([])
        self.assertEqual(result, [])

    def test_backward_compat_alias(self):
        """detect_event_classes is an alias for detect_classes."""
        records = [{"class_label": "not_humanitarian"}]
        self.assertEqual(detect_event_classes(records), detect_classes(records))
    ...
    def test_load_tsv_reads_correctly(self):
        ...
        try:
            self._write_tsv(path, [
                ["1", "hello world", "affected_individuals"],
...
class TestLoadPseudoLabelsIO(unittest.TestCase):
    ...
    def test_load_pseudo_labels(self):
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".csv", delete=False) as f:
            ...
            writer.writerow(["1", "hello", "affected_individuals",

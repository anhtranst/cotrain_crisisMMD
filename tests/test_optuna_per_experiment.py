...
def _make_result(task="humanitarian", modality="text_only", budget=5, seed_set=1, **overrides):
...
class TestBestParamsSaved(unittest.TestCase):
    ...
    def test_saves_best_params_json(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            best_path = Path(tmpdir) / "humanitarian" / "text_only" / "5_set1" / "trials_3" / "best_params.json"
            ...
            self.assertEqual(saved["task"], "humanitarian")
    ...
    def test_skips_existing_study(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            out_dir = Path(tmpdir) / "humanitarian" / "text_only" / "5_set1" / "trials_15"
...
class TestFindLatestTrials(unittest.TestCase):
    ...
    def test_finds_highest(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            experiment_dir = Path(tmpdir) / "humanitarian" / "text_only" / "5_set1"
...
class TestAllStudiesSkipsCompleted(unittest.TestCase):
    ...
    def test_partial_skip(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            out_dir = Path(tmpdir) / "humanitarian" / "text_only" / "5_set1" / "trials_10"
            out_dir.mkdir(parents=True)
            existing = {
                "event": "test_event", "budget": 5, "seed_set": 1,
                "status": "done", "best_params": {"lr": 0.001},
